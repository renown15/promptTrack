.PHONY: dev dev-api dev-web dev-log logs db db-stop db-logs db-studio db-dump db-restore db-reset migrate migrate-dev push test test-unit test-integration test-e2e lint lint-report typecheck build clean stop stop-dev prod prod-stop prod-logs prod-autostart prod-autostart-off ci help
...
LAUNCHAGENT_SRC := $(CURDIR)/scripts/com.prompttrack.prod.plist
LAUNCHAGENT := $(HOME)/Library/LaunchAgents/com.prompttrack.prod.plist
...
prod: ## Build and start production stack (postgres + api + web) — note: conflicts with dev on same ports
	docker compose -f docker-compose.prod.yml up -d --build
	@echo "$(CYAN)Production stack running$(RESET) → http://localhost:5173 | api: http://localhost:3051"
...
prod-autostart: ## Install LaunchAgent — prod stack starts automatically on login
	sed 's|__REPO_ROOT__|$(CURDIR)|g' $(LAUNCHAGENT_SRC) > $(LAUNCHAGENT)
	launchctl load $(LAUNCHAGENT)
	@echo "$(CYAN)Autostart enabled$(RESET) — prod stack will start when Docker Desktop opens"

prod-autostart-off: ## Remove LaunchAgent autostart
...
stop-dev: ## Kill dev servers (API + web) and stop Docker services
	@lsof -ti :$(API_PORT) | xargs kill -9 2>/dev/null || true
	@lsof -ti :$(WEB_PORT) | xargs kill -9 2>/dev/null || true
	docker-compose down
	@echo "Dev servers and Docker stopped."

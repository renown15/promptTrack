...
export function computeHealth(
  ...
  for (const file of files) {
    for (const name of metricNames) {
      const h = health[name];
      if (!h) continue;
      // Override supersedes raw metric for health counts.
      // accepted/rejected are treated as resolved (green).
      const override = file.overrides?.[name];
      if (override) {
        const s = override.status;
        if (s === "red") h.red++;
        else if (s === "amber") h.amber++;
        else h.green++; // green, accepted, rejected → resolved
        continue;
      }
...
export function fileMatchesFilter(
  ...
  const override = file.overrides?.[filter.name];
  if (override) {
    if (filter.status === "error") return false;
    return override.status === filter.status;
  }

...
export function filterTreeTableFiles(
  ...
  if (activeFilter) {
    result = result.filter((f) => {
      ...
      // If an override exists for this metric, use override status (accepted/rejected
      // are treated as resolved — don't include in raw-status filter buckets).
      const override = f.overrides?.[activeFilter.name];
      if (override) {
        if (activeFilter.status === "error") return false;
        return override.status === activeFilter.status;
      }

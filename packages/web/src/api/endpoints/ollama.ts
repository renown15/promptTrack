...
export const ollamaApi = {
  getLlmEnabled: async (): Promise<boolean> => {
    const r = await apiClient.get<{ enabled: boolean }>(
      "/settings/llm-enabled"
    );
    return r.data.enabled;
  },

  setLlmEnabled: async (enabled: boolean): Promise<boolean> => {
    const r = await apiClient.put<{ enabled: boolean }>(
      "/settings/llm-enabled",
      { enabled }
    );
    return r.data.enabled;

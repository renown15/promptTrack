 ...
 *   override accepted/rejected 0  (human or agent has reviewed — don't penalise)
 *   override green            0
 *   override amber           +2
 *   override red             +5
 */
export function calculateProblemScore(
  ...
  for (const [name, value] of Object.entries(metrics)) {
    ...
    if (override) {
      // Override supersedes raw metric. accepted/rejected mean reviewed — no penalty.
      if (override.status === "red") score += 5;
      else if (override.status === "amber") score += 2;
      continue;
    }
...
export function serializeState(state: CollectionInsightState) {
  return {
    files: Array.from(state.files.values()).map((f) => ({
      ...
      problemScore: calculateProblemScore(f.metrics, f.overrides, f.gitStatus),
    })),
